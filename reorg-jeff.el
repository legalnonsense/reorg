...
;; (jrf/reorg-agenda-cases* nil)
;; (jrf/reorg-agenda-cases* t)

(defun jrf/reorg-agenda-cases* (&optional hide-archives)
  (interactive "P")
  (reorg-org-capture-enable)
  (reorg-open-sidebar
   `( :sources ((org . ,reorg-test-org-file-list))
				  ...
				  (.todo . (lambda (a b)
					     (reorg--sort-by-list a
								  ...
								  '("TASK"
								    "DELEGATED"
								    "WAITING"))))))
		 ( :group (when (and (member .todo '("EVENT"
						     "DEADLINE"
						     "OPP_DUE"))
				     (not .archivedp))
			    "--CALENDAR--")
		   :sort-results ((.ts-single-active . reorg-string<)))
		 ( :group (when (and (not (quote ,hide-archives))
				     .archivedp
				     (or .ts-closed
					 .ts-active-first
					 .deadline
		   ...
		   :format-results ((s-pad-right 20
						 ...
						 (if-let ((ts (or .ts-closed
						     ...
						     (reorg-org--format-time-string
						      ts "%b %e, %Y")
						   " "))
				    (reorg--truncate-and-pad .headline
							     47
							     50
							     "...")
				    .clocked-time)
