...
There is already a lot going on here. One of the key features is that entries can appear in multiple places in the outline.
...
*** _Not just for orgmode_

Reorg isn’t just for org-mode, so let’s add some more:
